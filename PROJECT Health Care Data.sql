...
(SELECT `Medical Condition`, COUNT(*) AS condition_count, YEAR(`Date of Admission`) AS year_admission,
ROW_NUMBER() OVER (PARTITION BY YEAR(`Date of Admission`) ORDER BY COUNT(*) DESC) AS row_num
FROM healthcare_dataset_staging
GROUP BY `Medical Condition`, YEAR(`Date of Admission`)
ORDER BY year_admission)
SELECT `Medical Condition`, condition_count, year_admission
FROM condition_year
WHERE row_num = 1;
-- Based on my findings, there were 28.697 patients admitted and diagnosed with a medical condition. It seems every year has a different medical condition, except for 2019 and 2024, both have asthma. 
